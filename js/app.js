...
// --- AI CONSULTANT COMPONENT (UPDATED: Bigger Button, Better Z-Index) ---
const AIConsultant = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    // window.sendLeadToTelegram data.js faylidan keladi
    try {
        ...
        if (success) {
            ...
            alert("Xatolik bo'ldi. Internetni tekshiring.");
        }
    } catch (e) {
        console.error(e);
    ...
    <div className="fixed bottom-6 right-6 md:bottom-10 md:right-10 z-[9999]">
      {!isOpen ? (
        <button 
          ...
          className="w-20 h-20 bg-black text-white rounded-[28px] shadow-[0_20px_50px_rgba(0,0,0,0.3)] flex items-center justify-center hover:scale-105 active:scale-95 transition-all border border-white/10 group animate-spring"
        >
          <i className="fas fa-comment-dots text-3xl group-hover:rotate-12 transition-transform"></i>
          <span className="absolute -top-1 -right-1 w-4 h-4 bg-blue-500 rounded-full border-2 border-black animate-pulse"></span>
        </button>
      ) : (
        <div className="w-[90vw] sm:w-[380px] md:w-[420px] bg-white rounded-[32px] shadow-[0_40px_100px_rgba(0,0,0,0.25)] border border-gray-100 flex flex-col overflow-hidden animate-spring max-h-[80vh] fixed bottom-6 right-6 md:bottom-10 md:right-10 z-[9999]">
          {/* Header */}
          <div className="p-5 md:p-6 bg-black text-white flex items-center justify-between shrink-0">
            <div className="flex items-center space-x-3 md:space-x-4">
              ...
              <div>
                <h4 className="font-bold text-sm tracking-tight">Million KM Support</h4>
          ...
          {/* Chat Body */}
          <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6 ios-scroll bg-[#F5F5F7]">
            
            <div className="space-y-6">
              <div className="flex justify-start animate-fade-up">
                <div className="max-w-[85%] bg-white p-5 rounded-2xl rounded-tl-none border border-gray-200 shadow-sm text-sm font-semibold text-slate-800 leading-relaxed">
                  Assalomu alaykum! Million KM servisiga xush kelibsiz.
                </div>
              </div>

              {(step === 'model' || step === 'problem' || step === 'contact' || step === 'success') && (
                <div className="flex justify-start animate-fade-up">
                  <div className="max-w-[85%] bg-white p-5 rounded-2xl rounded-tl-none border border-gray-200 shadow-sm text-sm font-semibold text-slate-800">
                    Mashinangiz modeli? (Masalan: Cobalt)
                  </div>
                </div>
              )}
              ...
              {step === 'model' && (
                <div className="space-y-3 animate-fade-up">
                  <input 
                    autoFocus
                    className="w-full h-14 px-6 bg-white border border-gray-200 rounded-2xl text-base font-bold focus:ring-2 focus:ring-blue-500 outline-none transition-all shadow-sm"
                    placeholder="Yozing..."
                    value={data.model}
                    onChange={e => setData({...data, model: e.target.value})}
                    onKeyPress={e => e.key === 'Enter' && data.model && handleNext()}
                  />
                  <button onClick={handleNext} disabled={!data.model} className="w-full h-14 bg-black text-white rounded-2xl font-bold text-xs uppercase tracking-widest disabled:opacity-50 transition-all">
                    Davom etish
                  </button>
                </div>
              )}
              ...
              {(step === 'problem' || step === 'contact' || step === 'success') && (
                <div className="flex justify-start animate-fade-up">
                  <div className="max-w-[85%] bg-white p-5 rounded-2xl rounded-tl-none border border-gray-200 shadow-sm text-sm font-semibold text-slate-800">
                    Qanday muammo bor yoki nima xizmat kerak?
                  </div>
                </div>
              )}
              ...
              {step === 'problem' && (
                <div className="space-y-3 animate-fade-up">
                  <textarea 
                    autoFocus
                    className="w-full p-5 bg-white border border-gray-200 rounded-2xl text-base font-bold focus:ring-2 focus:ring-blue-500 outline-none transition-all min-h-[100px] resize-none shadow-sm"
                    placeholder="Masalan: moy almashtirish..."
                    value={data.problem}
                    onChange={e => setData({...data, problem: e.target.value})}
                  />
                  <button onClick={handleNext} disabled={!data.problem} className="w-full h-14 bg-black text-white rounded-2xl font-bold text-xs uppercase tracking-widest disabled:opacity-50 transition-all">
                    Keyingi
                  </button>
                </div>
              )}
              ...
              {step === 'contact' && (
                <form onSubmit={handleSubmit} className="space-y-4 animate-fade-up">
                  <input required className="w-full h-14 px-6 bg-white border border-gray-200 rounded-2xl text-base font-bold outline-none focus:ring-2 focus:ring-blue-500 shadow-sm" placeholder="Ismingiz" value={data.name} onChange={e => setData({...data, name: e.target.value})} />
                  <input required type="tel" className="w-full h-14 px-6 bg-white border border-gray-200 rounded-2xl text-base font-bold outline-none focus:ring-2 focus:ring-blue-500 shadow-sm" placeholder="+998" value={data.phone} onChange={e => setData({...data, phone: e.target.value})} />
                  <button 
                    type="submit"
                    disabled={isLoading}
                    className="w-full h-16 bg-blue-600 text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl shadow-blue-500/20 active:scale-95 transition-all"
                  >
              ...
              {step === 'success' && (
                <div className="text-center py-8 animate-fade-up">
                  ...
                  <h3 className="text-xl font-black tracking-tight text-slate-900">Xabar ketdi!</h3>
          ...
          <div className="bg-white/95 backdrop-blur-3xl rounded-[32px] p-10 w-full max-w-md shadow-2xl relative border border-white/50 animate-spring">
            ...
            <div className="space-y-5">
               <div className="space-y-2">
                 ...
                 <input type="text" value={loginForm.name} onChange={e => setLoginForm({...loginForm, name: e.target.value})} placeholder="Masalan: Azizbek" className="w-full h-16 px-6 rounded-2xl bg-gray-50 border-none outline-none focus:ring-2 focus:ring-blue-500 font-bold text-lg" />
               </div>
               {loginForm.name.toLowerCase() !== 'admin' ? (
                  <div className="space-y-2">
                    ...
                    <input type="tel" value={loginForm.phone} onChange={e => setLoginForm({...loginForm, phone: e.target.value})} placeholder="+998" className="w-full h-16 px-6 rounded-2xl bg-gray-50 border-none outline-none focus:ring-2 focus:ring-blue-500 font-bold text-lg" />
                  </div>
               ) : (
                  <div className="space-y-2">
                    <label className="text-[11px] font-bold text-gray-400 uppercase tracking-widest ml-1">Maxfiy kod</label>
                    <input type="password" value={loginForm.password} onChange={e => setLoginForm({...loginForm, password: e.target.value})} placeholder="••••" className="w-full h-16 px-6 rounded-2xl bg-gray-50 border-none outline-none focus:ring-2 focus:ring-blue-500 font-bold text-center tracking-[0.5em] text-lg" />
                  </div>
               )}
